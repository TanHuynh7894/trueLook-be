...
export class UserRolesService {
  ...
  async create(createUserRoleDto: CreateUserRoleDto) {
    ...
    if (!user) throw new NotFoundException(`User with id ${createUserRoleDto.user_id} not found`);

    const role = await this.rolesRepository.findOneBy({ id: createUserRoleDto.role_id });
    if (!role) throw new NotFoundException(`Role with id ${createUserRoleDto.role_id} not found`);
    ...
    if (existingUserRole) {
      throw new ConflictException(`User role (${createUserRoleDto.user_id}, ${createUserRoleDto.role_id}) already exists`);
    }
  ...
  async remove(userId: string, roleId: string) {
    const result = await this.userRolesRepository.delete({ user_id: userId, role_id: roleId });
    if (result.affected === 0) {
      throw new NotFoundException(`User role (${userId}, ${roleId}) not found for delete`);
    }
    return { message: `Deleted user role (${userId}, ${roleId})`, statusCode: 200 };
  }

  async assignRoleByName(userId: string, roleName: string) {
    const role = await this.rolesRepository.findOneBy({ name: roleName });
    if (!role) throw new NotFoundException(`Role với tên "${roleName}" không tồn tại trong hệ thống!`);

    const existingUserRole = await this.userRolesRepository.findOneBy({ user_id: userId, role_id: role.id });
    if (existingUserRole) return existingUserRole;

    const newUserRole = this.userRolesRepository.create({ user_id: userId, role_id: role.id });
    return await this.userRolesRepository.save(newUserRole);
  }
